...
class AssgnTask2{
    ...
    public static Integer[][] compressMatrix( Integer[][] matrix ){
        ...
        //TO DO
        Integer[][] new_m = new Integer [matrix.length/2][matrix.length/2];
        for(int col=0; col<matrix.length; col+=2){
            for(int row=0; row<matrix[0].length; row+=2){
                new_m[col/2][row/2] = matrix[col][row] + matrix[col][row+1] + matrix[col+1][row] + matrix[col+1][row+1];
